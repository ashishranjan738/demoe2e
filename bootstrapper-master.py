...
def create_init_directory(build_number):
    date = str(datetime.now())[:10]
    path = os.path.expanduser('~')+"/e2e/%s/%s" % (date, build_number)
...
def create_platform_yml(plans, args):
    ...
    for platform in plans['Platform']:
        ...
        for platform_name, platform_info in platform.items():
            ...
            platform_yml = {'Testrail_project_id': project_id, 'Build_date': args['date'], 'Build_number': args['build_number'],
                            'Platform_name': platform_name,
                            'Build_time': str(datetime.now())[12:17], 'Description': platform_info['Description'],
                            'Test_suite': platform_info['Test Suite']}
...
def main():
    ...
    args['base_path'], err, args['date'] = create_init_directory(str(args['build_number']))
    if err == -1:
